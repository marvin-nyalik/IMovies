...
const apiKey = '8xeEchA0ElEY84MtPMbN';
...
const getSingleMovieComments = async (movieId, commentsCont, count) => {
  ...
  try {
    ...
    if (allComments.ok) {
      const allCommentsData = await allComments.json();
      counter.innerHTML = `(${commentCounter(allCommentsData)})`;
...
export {
  getShows, postLikes, getLikes, apiKey, addMovieComment, getSingleMovieComments, getShowDetails,
};